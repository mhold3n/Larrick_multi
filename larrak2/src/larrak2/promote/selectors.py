...
def _survive_ref_dirs(
    ...
    all_indices = np.concatenate([current_indices, candidate_indices]) if len(current_indices) > 0 else candidate_indices
    F_all = F[all_indices]
    ...
    def associate(indices):
        ...
        best_ref_idx = np.argmax(cosine, axis=1) # (N,)
        dist = np.linalg.norm(vals - (p_norm * p_unit), axis=1) # wait, math.
    ...
    for _ in range(n_needed):
        ...
        valid_refs = [r for r in range(len(ref_dirs)) if r in candidates_by_niche and len(candidates_by_niche[r]) > 0]
...
def select_hybrid(
    ...
    if k_exploit < 0: k_exploit = 0
    if k_explore < 0: k_explore = 0
