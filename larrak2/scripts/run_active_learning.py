...
def main():
    ...
    parser.add_argument("--dry_run", action="store_true", help="Do not actually run solvers, just generate plan")
    ...
    for c in selected:
        print(f"  Candidate {c['index']}: Unc={c['u_total']:.4f} (G={c['u_gear']:.4f}, S={c['u_scavenge']:.4f})")
    ...
    foam_runner = OpenFoamRunner(template_case=template_foam)
