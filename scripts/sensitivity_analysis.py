...
from larrak2.core.encoding import decode_candidate, encode_candidate, bounds, mid_bounds_candidate
...
def sensitivity_scan(x_base: np.ndarray, ctx: EvalContext) -> dict:
    ...
    cand_base = decode_candidate(x_base)
    ...
    thermo_names = ["compression_duration", "expansion_duration", "heat_release_center", "heat_release_width"]
    # Gear parameters are indices 4-11
    gear_names = ["base_radius", "pitch_coeff_0", "pitch_coeff_1", "pitch_coeff_2", 
                  "pitch_coeff_3", "pitch_coeff_4", "pitch_coeff_5", "pitch_coeff_6"]
    ...
    for i, name in enumerate(all_names):
        ...
        for metric in ["efficiency", "loss", "p_max", "W"]:
            ...
            if base_metrics[metric] != 0 and x_base[i] != 0:
                sens[f"sens_{metric}"] = float((d_plus - d_minus) / (2 * base_metrics[metric]) / (delta / x_base[i]))
            else:
...
def constraint_activation_report(X: np.ndarray, ctx: EvalContext) -> dict:
    ...
    for x in X:
        ...
        metrics_all.append({
            ...
            "ratio_error_max": gear_diag.get("ratio_error_max", 0) if "ratio_error_max" in gear_diag else 0,
    ...
    for j in range(G_all.shape[1]):
        ...
        stat = {
            "constraint": f"g{j}",
            "description": list(limits.values())[j]["desc"] if j < len(limits) else f"constraint {j}",
    ...
    metric_stats = {
        ...
        "p_max_bar": {
            ...
            "limit": 500.0,
            "headroom_pct": float((500 - np.max([m["p_max_bar"] for m in metrics_all])) / 500 * 100),
        },
    }
...
def main():
    ...
    if pareto_file.exists():
        ...
        X_pareto = np.vstack([mid_bounds_candidate()] + [xl + rng.random(len(xl)) * (xu - xl) for _ in range(15)])
    ...
    for stat in activation["constraint_stats"]:
        print(f"{stat['constraint']:<12} {stat['description']:<30} {stat['mean']:+.4f} "
    ...
    max_thermo_eff_sens = max(abs((s["d_efficiency_plus"] - s["d_efficiency_minus"]) / 2) for s in thermo_sens)
