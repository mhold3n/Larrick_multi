...
def main():
    ...
    if not changed:
        for item in os.listdir("."):
         ...
         for p in changed:
            tl = top_level_dir(p)
            if tl and os.path.isdir(tl): # Only check if it looks like a directory or path implies one
        ...
        for p in changed:
            ...
            if tl not in allowed:
                ...
                if os.path.exists(tl): # and os.path.isdir(tl) - strictly allow files?
                     ...
                     if os.path.isdir(tl):
                        print(f"[LAYOUT] Forbidden top-level directory: '{tl}' (path: {p})", file=sys.stderr)
                        sys.exit(1)
    ...
    for d in sorted(created_dirs, key=len):
        ...
        for pat in banned_patterns:
            if pat.match(name):
                ...
                if d in allowed:
                    continue
                print(f"[LAYOUT] Banned directory name '{name}' matched '{pat.pattern}' (dir: {d})", file=sys.stderr)
                sys.exit(1)
        ...
        if top_level_dir(d) == PurePosixPath(d).parts[0]:
            ...
            if name != output_dir and re.match(r"^(out|output|outputs)[-_ ]?\d*$", name):
                 ...
                 if name in allowed and name == output_dir:
                     continue
                 print(f"[LAYOUT] Output directory must be '{output_dir}', not '{name}'", file=sys.stderr)
                 sys.exit(1)
