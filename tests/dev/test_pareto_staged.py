...
def run_tiny_workflow(outdir: Path, seed: int):
    ...
    workflow = StagedWorkflow(outdir=outdir, rpm=3000.0, torque=200.0, seed=seed)
...
def test_staged_outputs(temp_staged_dir):
    ...
    # Check files exist
    assert (
        temp_staged_dir / "stage1"
    ).exists()  # Pymoo uses checkpointing? No, we didn't enable it explicitly.
