...
from larrak2.thermo.valve_timing import (
    breathing_with_derived_timing,
    derive_valve_timing,
    overlap_duration_deg,
)
...
def test_openfoam_feature_payload_uses_candidate_relative_timing() -> None:
    params = ThermoParams(
        compression_duration=60.0,
        expansion_duration=95.0,
        heat_release_center=15.0,
        heat_release_width=30.0,
        lambda_af=1.05,
        intake_open_offset_from_bdc=-4.0,
        intake_duration_deg=84.0,
        exhaust_open_offset_from_expansion_tdc=-4.0,
        exhaust_duration_deg=92.0,
        spark_timing_deg_from_compression_tdc=-8.0,
    )
    ctx = EvalContext(rpm=2000.0, torque=80.0, fidelity=2)
    features = _openfoam_feature_payload(
        params=params,
        ctx=ctx,
        breathing=BreathingConfig(valve_timing_mode="candidate"),
    )

    assert features["intake_open_deg"] == -4.0
    assert features["intake_close_deg"] == 80.0
    assert features["exhaust_open_deg"] == -4.0
    assert features["exhaust_close_deg"] == 88.0


def test_scavenging_pressure_seed_tracks_open_ports() -> None:
    theta = np.linspace(0.0, 359.0, 360, dtype=np.float64)
    ...
    breathing = breathing_with_derived_timing(BreathingConfig(), derived)

    p_seed = _build_scavenging_pressure_seed(
        theta_deg=theta,
        volume=volume,
        breathing=breathing,
        gamma_u=1.35,
    )
    intake_mask = (theta >= derived.intake_open_deg) & (theta <= derived.intake_close_deg)
    exhaust_mask = (theta >= 0.0) & (theta <= derived.exhaust_close_deg)

    assert np.min(p_seed[intake_mask]) < breathing.p_manifold_Pa
    assert np.max(p_seed[exhaust_mask]) > breathing.p_back_Pa
